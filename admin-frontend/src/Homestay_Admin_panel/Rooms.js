...
const RoomManagement = () => {
  ...
  const handleSubmit = async (e, index) => {
    ...
    const roomRef = doc(db, 'Homestays', homestayId, 'Rooms', room.id || doc(collection(db, 'Homestays', homestayId, 'Rooms')).id);
  ...
  const removeRoom = async (index) => {
    ...
    if (roomToDelete.id) {
      try {
        await deleteDoc(doc(db, 'Homestays', homestayId, 'Rooms', roomToDelete.id));
  ...
  const calculateAndStoreMaxGuests = async () => {
    ...
    try {
      await updateDoc(doc(db, 'Homestays', homestayId), { totalMaxGuests });
    } catch (error) {
